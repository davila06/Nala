...
{
    [Fact]
    public async Task Handle_OwnedPetWithoutPhoto_CreatesPendingOwnerSubmission()
    {
        ...
        await adoptions.Received(1).AddAnimalAsync(
            Arg.Is<AdoptablePet>(animal => animal.Status == AdoptionStatus.PendingReview && animal.Source == AdoptionSource.Owner && animal.PhotoUrls.Count == 0),
