...
import { apiLogin, registerCollarSerial, uniqueSerial } from "./fixtures/api-setup";
...
test.describe("Admin — Collar tag dashboard", () => {
  test("metrics render and bulk mark-sold succeeds for a searched serial", async ({ page }) => {
    ...
    await expect(page.locator("p", { hasText: "Sin activar" }).first()).toBeVisible();
