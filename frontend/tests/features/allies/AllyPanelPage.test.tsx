...
describe("AllyPanelPage", () => {
  ...
  it("shows the application form when the user has not applied yet", async () => {
    ...
    expect(
      await screen.findByRole("heading", {
        name: /red de aliados verificados/i,
      }),
    ).toBeInTheDocument();
  ...
  it("shows the verified ally inbox and confirms an operational action", async () => {
    ...
    server.use(
      ...
      http.put("http://localhost:5000/api/allies/me/alerts/:id/action", () => {
        confirmAction();
    ...
    expect(
      await screen.findByText(
        /alerta operativa: nala necesita apoyo en tu zona/i,
      ),
    ).toBeInTheDocument();

    await user.click(
      screen.getByRole("button", { name: /ya buscamos en nuestra área/i }),
    );
