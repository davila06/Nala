...
export function useConfirmBountyDeposit() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (depositReference: string) =>
      bountyApi.confirmDeposit(depositReference),
    onSuccess: () => {
...
export function useReleaseBounty() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (bountyId: string) => bountyApi.release(bountyId),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["bounty"] });
    },
  });
