...
import { useBountyForEvent, useConfirmBountyDeposit, useCreateBounty } from '../hooks/useBounty'

const SINPE_NUMBER = '7000-0000'
    ...
    return (
      <div className="rounded-2xl border border-dashed border-warn-300 bg-warn-50 p-4">
        ...
        <p className="mt-0.5 text-xs text-warn-600">
          La recompensa aparece en el mapa y motiva a la red a buscar activamente.
          PawTrack retiene un 10% de comisión al liberarla.
        </p>
        {!showCreate ? (
          <button
            type="button"
          ...
          <div className="mt-3 space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => setShowCreate(false)} className="text-xs text-sand-500 hover:text-sand-700">
                Cancelar
              </button>
            </div>
          </div>
    ...
    <AnimatePresence>
      ...
      >
        <div className="flex items-start justify-between gap-3">
          ...
          <span className={`rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide ${statusColor[bounty.status] ?? 'bg-sand-100 text-sand-500'}`}>
