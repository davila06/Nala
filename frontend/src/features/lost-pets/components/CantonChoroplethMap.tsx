...
export function CantonChoroplethMap({
  cantonRecovery,
  maxRecoveryRate,
}: CantonChoroplethMapProps) {
  ...
  if (isError) {
    return (
      ...
      >
        ...
        Mapa de cantones no disponible temporalmente
      </div>
    );
  }

  // Return null when map data unavailable — parent section still renders
  if (!geoJson && !isLoading) return null;
