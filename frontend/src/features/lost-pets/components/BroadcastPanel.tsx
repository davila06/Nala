        ...
        <span className="flex items-center gap-1 text-xs font-semibold text-rescue-700">
          ...
          {attempt.trackingClicks > 0 && (
            <span className="ml-2 rounded-full bg-rescue-100 px-2 py-0.5 text-xs text-rescue-800">
              {attempt.trackingClicks} clic
              {attempt.trackingClicks !== 1 ? "s" : ""}
            </span>
          )}
        </span>
      </div>
...
export function BroadcastPanel({
  ...
  const { data: status, isLoading: isLoadingStatus } =
    useBroadcastStatus(lostEventId);
  const {
    mutate: trigger,
    isPending: isTriggering,
    isError,
    error,
  } = useTriggerBroadcast(lostEventId);
  ...
  return (
    ...
    >
      <h2 className="mb-1 text-sm font-bold text-trust-900">
        <span aria-hidden="true">📡</span> Difusión multicanal
      </h2>
      <p className="mb-4 text-xs text-trust-700">
        Notifica automáticamente a través de todos los canales configurados
        (correo, WhatsApp, Telegram y Facebook) para ampliar el alcance de la
        búsqueda.
      </p>
      ...
      {isLoadingStatus ? (
        <div className="mb-4 space-y-2">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="h-10 animate-pulse rounded-xl bg-trust-100"
            />
          ))}
        </div>
      ) : hasBroadcast ? (
        ...
        <p className="mb-4 text-xs text-trust-600">
          Aún no se ha enviado ninguna difusión. Usa el botón de abajo para
      ...
      <button
        ...
        aria-label={
          hasBroadcast
            ? "Volver a difundir por todos los canales"
            : "Difundir por todos los canales"
        }
      >
        {isTriggering ? (
          <>
