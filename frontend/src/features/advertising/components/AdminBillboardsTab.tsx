...
import type { BillboardDto, BillboardPlacement } from "@/features/advertising/api/billboardsApi";

const PLACEMENTS: BillboardPlacement[] = ["Map", "Dashboard", "Directory", "Feed"];
...
function BillboardRow({ b }: { b: BillboardDto }) {
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    e.target.value = "";
    if (!file) return;
    if (file.size > MAX_BYTES) { toast.error("Máximo 5 MB."); return; }
    uploadImg.mutate({ id: b.id, file }, {
    ...
    <li className="rounded-xl border border-sand-100 bg-surface p-4 space-y-2">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <p className="font-semibold text-ink-900 text-sm truncate">{b.title}</p>
            <span className={`text-[10px] font-bold rounded-full px-2 py-0.5 ${STATUS_COLORS[b.status] ?? ""}`}>
              {b.status}
            </span>
            <span className="text-[10px] text-sand-500">{PLACEMENT_LABELS[b.placement] ?? b.placement}</span>
          </div>
          {b.body && <p className="text-xs text-sand-500 line-clamp-2">{b.body}</p>}
      ...
      <div className="flex gap-2 flex-wrap">
        {b.status !== "Active" && b.status !== "Expired" && (
          <Button size="sm" onClick={() => setStatus.mutate({ id: b.id, status: "active" }, {
            onSuccess: () => toast.success("Activada"),
          })}>Activar</Button>
        )}
        {b.status === "Active" && (
          <Button size="sm" variant="secondary" onClick={() => setStatus.mutate({ id: b.id, status: "paused" }, {
...
function CreateBillboardForm({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = () => {
    create.mutate(
      { ...form, body: form.body || undefined, ctaLabel: form.ctaLabel || undefined,
        ctaUrl: form.ctaUrl || undefined,
        startsAt: new Date(form.startsAt).toISOString(),
        endsAt: new Date(form.endsAt).toISOString() },
      { onSuccess: () => { toast.success("Valla creada"); onClose(); },
  ...
  return (
    <div className="rounded-2xl border border-brand-200 bg-brand-50 p-4 space-y-3">
      <h3 className="text-sm font-semibold text-brand-800">Nueva valla publicitaria</h3>
      <div className="grid grid-cols-2 gap-3">
        <div className="col-span-2">
          <label className="mb-1 block text-xs font-medium text-sand-600">Título *</label>
          <input {...field("title")} placeholder="Cuida a tu mascota con..."
        ...
        <div className="col-span-2">
          ...
          <textarea {...field("body")} rows={2}
            className="w-full rounded-xl border border-sand-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" />
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-sand-600">Ubicación *</label>
          <select {...field("placement")}
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-sand-600">Prioridad (0-100)</label>
          <input type="number" min="0" max="100" {...field("priority")}
            className="w-full rounded-xl border border-sand-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" />
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-sand-600">Inicio</label>
          <input type="datetime-local" {...field("startsAt")}
            className="w-full rounded-xl border border-sand-200 px-3 py-2 text-sm" />
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-sand-600">Fin</label>
          <input type="datetime-local" {...field("endsAt")}
            className="w-full rounded-xl border border-sand-200 px-3 py-2 text-sm" />
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-sand-600">CTA texto</label>
          <input {...field("ctaLabel")} placeholder="Ver más →"
            className="w-full rounded-xl border border-sand-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" />
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-sand-600">CTA URL</label>
          <input {...field("ctaUrl")} placeholder="https://..."
            className="w-full rounded-xl border border-sand-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" />
        </div>
      </div>
      <div className="flex gap-2">
        <Button onClick={handleSubmit} loading={create.isPending} size="sm">Crear valla</Button>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-ink-900">🪧 Vallas publicitarias</h2>
        {!showForm && (
          <Button size="sm" onClick={() => setShowForm(true)}>+ Nueva valla</Button>
        )}
      </div>
      ...
      <ul className="space-y-3">
        {(data?.items ?? []).map((b) => <BillboardRow key={b.id} b={b} />)}
      </ul>
    </div>
  );
}
