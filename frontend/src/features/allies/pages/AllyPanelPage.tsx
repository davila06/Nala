...
export default function AllyPanelPage() {
  ...
  const { mutate: submitApplication, isPending: isSubmitting } =
    useSubmitAllyApplication();
  const { mutate: confirmAction, isPending: isConfirmingAction } =
    useConfirmAllyAlertAction();
  const isVerified = profile?.verificationStatus === "Verified";
  const { data: alerts, isLoading: isAlertsLoading } =
    useMyAllyAlerts(isVerified);
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 animate-fade-in-up">
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-rescue-700">
      ...
      {!isProfileLoading && !isVerified && (
        <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <form
            ...
            className="rounded-3xl border border-sand-200 field-input p-6 shadow-sm"
          >
            <div className="mb-6">
          ...
          <aside className="rounded-3xl bg-trust-900 p-6 text-white shadow-sm">
            ...
            <h2 className="mt-3 text-2xl font-black tracking-tight">
              Verificación manual por el equipo PawTrack
            </h2>
            <p className="mt-3 text-sm text-trust-50/90">
              Cuando la solicitud quede aprobada, tu organización recibirá una
              bandeja operativa con alertas de pérdida dentro de su cobertura
              declarada.
            </p>
          </aside>
        </section>
            ...
            <div className="space-y-4">
              {alerts.map((alert) => (
                ...
                >
                  ...
                  {alert.actionSummary && (
                    <p className="mt-3 text-sm font-medium text-rescue-700">
                      {alert.actionSummary}
                    </p>
                  )}
                </article>
