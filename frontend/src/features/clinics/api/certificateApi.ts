...
export type CertificateType =
  | 'Vaccination' | 'GeneralExam' | 'Deworming'
  | 'Neutering' | 'HealthClearance' | 'MicrochipRegistration'
...
export const certificateApi = {
  getForPet: (petId: string) =>
    apiClient.get<CertificateDto[]>(`/certificates/pet/${petId}`).then((r) => r.data),

  verify: (code: string) =>
    apiClient.get<CertificateDto | null>(`/certificates/verify/${code}`).then((r) => r.data),
