...
export function NotificationCenter() {
  ...
  const { mutate: respondResolveCheck, isPending: isRespondingResolveCheck } =
    useRespondResolveCheck();
  const [searchParams, setSearchParams] = useSearchParams();
  const { status: pushStatus, subscribe: pushSubscribe } =
    usePushSubscription();

  const unreadCount = data?.totalCount ?? 0;
  const resolveCheckNotificationId = searchParams.get(
      ...
      {resolveCheckNotificationId && (
        ...
        >
          ...
          >
            <h2
              id="resolve-title"
              className="mt-1 text-lg font-extrabold text-sand-900"
            >
