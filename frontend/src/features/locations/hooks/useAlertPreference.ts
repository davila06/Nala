...
function readStoredAlerts(): boolean {
  try {
    return localStorage.getItem(PREFS_KEY_ALERTS) === "true";
  } catch {
    return false;
  }
}
...
function writeStoredAlerts(value: boolean) {
  try {
    localStorage.setItem(PREFS_KEY_ALERTS, String(value));
  } catch {
    /* ignore */
  }
}
...
export function useAlertPreference() {
  ...
  const [receiveNearbyAlerts, setReceiveNearbyAlerts] =
    useState(readStoredAlerts);
  const [quietHours, setQuietHoursState] = useState<QuietHoursValue | null>(
    readStoredQuiet,
  );
