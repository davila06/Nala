...
import { useNeighborStatus, useUpdateNeighborSettings } from "../hooks/useNeighbor";
...
export function NeighborStatusCard() {
  ...
  if (!status?.isEnrolled) {
    return (
      <>
        ...
        >
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-trust-100 text-xl" aria-hidden="true">
            🏘️
          </span>
          <div>
            <p className="text-sm font-semibold text-sand-900">Activar Guardia Vecinal</p>
            <p className="text-xs text-sand-500 mt-0.5">
              Recibe alertas cuando una mascota se pierde en tu cuadra
            </p>
          </div>
          <svg viewBox="0 0 16 16" fill="currentColor" className="ml-auto h-4 w-4 shrink-0 text-sand-300 mt-1" aria-hidden="true">
            <path fillRule="evenodd" d="M6.22 4.22a.75.75 0 0 1 1.06 0l3.25 3.25a.75.75 0 0 1 0 1.06l-3.25 3.25a.75.75 0 0 1-1.06-1.06L8.94 8 6.22 5.28a.75.75 0 0 1 0-1.06Z" clipRule="evenodd" />
          </svg>
        </button>
        <NeighborNetworkSetup isOpen={setupOpen} onClose={() => setSetupOpen(false)} />
      </>
    );
  }
  ...
  const toggleActive = () => {
    updateSettings.mutate(
      ...
      {
        onSuccess: () => toast.success(status.isActive ? "Guardia Vecinal desactivada" : "Guardia Vecinal activada"),
    ...
    <div className="rounded-2xl border border-trust-200 bg-trust-50 p-4 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-semibold text-trust-900">Guardia Vecinal</p>
        ...
        <button
          ...
          aria-label={status.isActive ? "Desactivar Guardia Vecinal" : "Activar Guardia Vecinal"}
