...
export function NeighborNetworkSetup({ isOpen, onClose }: NeighborNetworkSetupProps) {
  ...
  const handleSubmit = () => {
    ...
    enroll.mutate(
      ...
      {
        onSuccess: () => {
          toast.success("¡Guardia Vecinal activada! Recibirás alertas cuando haya mascotas perdidas cerca.");
          onClose();
        },
        onError: (err: unknown) =>
          toast.error(
  ...
  return (
    <Drawer isOpen={isOpen} onClose={onClose} title="Activar Guardia Vecinal 🏘️" side="bottom">
      <div className="space-y-5 pb-safe">

        {/* Benefit explanation */}
        <div className="rounded-xl border border-trust-200 bg-trust-50 p-4 space-y-2">
          <p className="text-sm font-semibold text-trust-800">¿Qué es la Guardia Vecinal?</p>
          <ul className="space-y-1.5 text-xs text-trust-700">
            <li className="flex items-start gap-1.5">
              <span aria-hidden="true" className="mt-0.5">🔔</span>
              Recibe alertas al instante cuando una mascota se pierde en tu cuadra
            </li>
            <li className="flex items-start gap-1.5">
              <span aria-hidden="true" className="mt-0.5">👀</span>
              Ayuda a vecinos a recuperar a sus mascotas — gana puntos como rescatador
            </li>
            <li className="flex items-start gap-1.5">
              <span aria-hidden="true" className="mt-0.5">🔒</span>
        ...
        {/* Phone input */}
        <div>
          <label htmlFor="neighbor-phone" className="mb-1 block text-xs font-medium text-sand-600">
          ...
          />
          {phoneError && (
            <p id="neighbor-phone-err" role="alert" className="mt-1 text-xs text-danger-600">
              {phoneError}
            </p>
          )}
          <p className="mt-1 text-xs text-sand-400">Formato: 8 dígitos o +506 XXXX-XXXX</p>
        </div>

        {/* Radius selector */}
        <div>
          <p className="mb-2 text-xs font-medium text-sand-600">Radio de alerta</p>
          <div className="grid grid-cols-1 gap-2">
            {RADIUS_OPTIONS.map((opt) => (
              ...
              >
                {opt.label}
                {radius === opt.value && (
                  <svg viewBox="0 0 16 16" fill="currentColor" className="h-4 w-4 text-brand-500" aria-hidden="true">
