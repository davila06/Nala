...
export function VisualMatchPanel() {
  ...
  // Auto-request location on mount so the user doesn't have to click the button
  useEffect(() => {
    handleGetLocation();
  }, [handleGetLocation]);
    ...
    <div className="mx-auto max-w-lg px-4 py-8">
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        <button
          ...
          className={[
            "mb-4 flex w-full items-center justify-center gap-2 rounded-xl border py-2.5 text-sm font-medium shadow-sm transition-colors disabled:opacity-70",
            geoStatus === "granted"
              ? "border-rescue-300 bg-rescue-50 text-rescue-700 cursor-default"
              : geoStatus === "denied"
        ...
        >
          ...
          {geoStatus === "granted" && "Ubicación capturada"}
          {geoStatus === "denied" && "Permiso denegado — clic para reintentar"}
          {geoStatus === "idle" && "📍 Usar mi ubicación (opcional)"}
        </button>
          ...
          {results.length === 0 ? (
            ...
            <>
              <p className="mb-3 text-sm font-semibold text-sand-700">
                {Math.min(visibleCount, results.length)} de {results.length}{" "}
                posible
