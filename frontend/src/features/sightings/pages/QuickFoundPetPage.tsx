...
function scoreLabel(s: number): { text: string; color: string; bg: string } {
  if (s >= 0.85) return { text: "Alta coincidencia", color: "text-rescue-700", bg: "bg-rescue-100" };
  if (s >= 0.72) return { text: "Posible coincidencia", color: "text-brand-700", bg: "bg-brand-100" };
  return { text: "Coincidencia leve", color: "text-sand-600", bg: "bg-sand-100" };
}
  ...
  return (
    <div className="space-y-5">
      ...
      >
        {preview ? (
          ...
          <>
            ...
            <p className="text-sm font-semibold text-sand-700">Toca para tomar o subir una foto</p>
      ...
      <p className="text-center text-xs text-sand-400">
        La foto no se guarda — solo se usa para comparar con los reportes activos.
      </p>
    </div>
  );
}
  ...
  if (matches.length === 0) {
    return (
      <div className="space-y-5 text-center">
        ...
        <div>
          <h2 className="font-display text-lg font-bold text-sand-900">Sin coincidencias encontradas</h2>
          <p className="mt-1 text-sm text-sand-500">
            No encontramos una mascota perdida que coincida con la foto.
            Puedes reportar tu avistamiento igualmente para que el dueño lo vea.
          </p>
        </div>
        <Button fullWidth onClick={onReportAnyway}>
          Reportar avistamiento igualmente
        </Button>
        <Button fullWidth variant="secondary" onClick={() => window.history.back()}>
          Subir otra foto
        </Button>
      </div>
    );
    ...
    <div className="space-y-4">
      <div>
        <h2 className="font-display text-lg font-bold text-sand-900">
          {matches.length === 1 ? "1 posible coincidencia" : `${matches.length} posibles coincidencias`}
        </h2>
            ...
            <li key={m.petId}>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-sand-900 truncate">{m.petName}</p>
                  <p className="text-xs text-sand-500">{SPECIES_LABEL[m.species] ?? m.species}</p>
                  <span className={`mt-1 inline-block rounded-full px-2 py-0.5 text-[10px] font-semibold ${lbl.bg} ${lbl.color}`}>
                    {lbl.text}
                  </span>
                  {m.distanceKm != null && (
                ...
                <svg viewBox="0 0 16 16" fill="currentColor" className="h-4 w-4 shrink-0 text-sand-300" aria-hidden="true">
      ...
      <div className="pt-2 border-t border-sand-100 space-y-2">
        <p className="text-xs text-sand-500 text-center">¿No es ninguna de estas?</p>
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-2xl border border-rescue-200 bg-rescue-50 p-4 flex items-center gap-3">
        {match.photoUrl && (
          <img src={match.photoUrl} alt={match.petName} className="h-14 w-14 rounded-xl object-cover shrink-0 border border-rescue-200" />
        )}
        <div>
          <p className="font-display text-base font-bold text-rescue-800">{match.petName}</p>
          <p className="text-xs text-rescue-600">{SPECIES_LABEL[match.species] ?? match.species}</p>
          <span className={`mt-0.5 inline-block rounded-full px-2 py-0.5 text-[10px] font-semibold ${lbl.bg} ${lbl.color}`}>
            {lbl.text}
          </span>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <h2 className="font-display text-lg font-bold text-sand-900">¿Qué quieres hacer?</h2>
      ...
      <div className="rounded-xl border border-brand-200 bg-brand-50 p-4 text-center space-y-2">
        <p className="text-xs font-semibold text-brand-800">¿Ayudas mascotas seguido?</p>
        <p className="text-xs text-brand-700">Crea una cuenta gratis y gana puntos como rescatador verificado.</p>
        <Link
          to="/register"
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Sube una foto de la mascota que encontraste y la IA buscará coincidencias con mascotas perdidas en Costa Rica en segundos." />
        <meta property="og:title" content="Encontré una mascota — PawTrack CR" />
        <meta property="og:description" content="¿Encontraste una mascota? Busca al dueño con reconocimiento visual por IA." />
        <meta property="og:type" content="website" />
        <meta property="og:image" content="https://pawtrack.cr/og-encontre.png" />
      </Helmet>
      ...
      <div className="min-h-dvh bg-sand-50">
        ...
        <header className="sticky top-0 z-10 border-b border-sand-200 bg-surface/95 backdrop-blur-sm">
          <div className="mx-auto flex h-14 max-w-md items-center gap-3 px-4">
            ...
            >
              <span aria-hidden="true" className="flex h-7 w-7 items-center justify-center rounded-lg bg-brand-500 text-sm text-white">🐾</span>
              PawTrack CR
            </Link>
        ...
        <div className="bg-gradient-to-br from-trust-900 to-trust-800 px-4 py-6 text-white text-center">
          ...
          <h1 className="font-display text-xl font-bold">¿Encontraste una mascota?</h1>
              ...
              return (
                <div key={s} className="flex flex-1 items-center">
                  <div className="flex-1 flex flex-col items-center">
                    <div className={`h-2 rounded-full w-full transition-colors ${isDone ? "bg-rescue-500" : isActive ? "bg-brand-500" : "bg-sand-200"}`} />
                    <span className="mt-1 text-[10px] text-sand-400">
                      {s === "upload" ? "Foto" : s === "results" ? "Resultados" : "Contacto"}
                    </span>
                  </div>
        ...
        <main className="mx-auto max-w-md px-4 py-6">
          {matchMutation.isError && (
            <div role="alert" className="mb-4 rounded-xl border border-danger-200 bg-danger-50 px-4 py-3 text-sm text-danger-700">
              {(matchMutation.error as { response?: { data?: { detail?: string } } })?.response?.data?.detail
          ...
          <AnimatePresence mode="wait" initial={false}>
            ...
            >
              ...
              {step === "results" && (
                <StepResults
                  matches={matches}
                  onSelect={(m) => { setSelected(m); setStep("confirm"); }}
