...
function SightingAutoMatchPanel({
  ...
  const {
    ...
    isLoading,
    isError,
  } = useVisualMatchBySighting(sightingId, lat ?? undefined, lng ?? undefined);
...
export default function ReportSightingPage() {
  ...
  const { mutateAsync: reportSighting, isPending } = useReportSighting(
    petId ?? "",
  );
  ...
  const [submittedSightingId, setSubmittedSightingId] = useState<string | null>(
    null,
  );
  ...
  // Auto-request location on mount — it's required to submit the sighting
  useEffect(() => {
    handleGetLocation();
  }, [handleGetLocation]);
      ...
      <form onSubmit={handleSubmit} className="space-y-5 px-5 py-6">
        ...
        <div className="rounded-2xl border border-sand-200 p-4">
          ...
          {locationStatus === "granted" && lat !== null && lng !== null ? (
            ...
            <p className="text-sm text-danger-600">
              El acceso a la ubicación fue denegado. Actívalo desde la
        ...
        >
          {isPending ? (
            ...
            <>
              <span aria-hidden="true">🐾</span> Enviar avistamiento
            </>
          )}
        </button>
      </form>
