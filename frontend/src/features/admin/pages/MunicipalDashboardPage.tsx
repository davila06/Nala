...
function TierBadge({ tier }: { tier: MunicipalTier }) {
  ...
  return (
    <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${cls[tier]}`}>
      {TIER_LABELS[tier]}
    </span>
  );
}
...
function CapturesTab({ tier, canton }: { tier: MunicipalTier; canton: string }) {
  ...
  const [form, setForm] = useState({
    canton, species: "", color: "", breed: "", estimatedAge: "", notes: "", collarChipNumber: "",
  });
  ...
  const handleRecord = () => {
    ...
    recordCapture.mutate(
      ...
      {
        onSuccess: () => { toast.success("Registro creado"); setShowNewForm(false); setForm({ canton, species: "", color: "", breed: "", estimatedAge: "", notes: "", collarChipNumber: "" }); },
  ...
  const handleBulkUpdate = (status: CapturedAnimalStatus) => {
    ...
    bulkUpdate.mutate(
      ...
      {
        onSuccess: (r) => { toast.success(`${r.updated} registros actualizados`); setSelectedIds(new Set()); },
        onError: (err: unknown) => toast.error((err as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? "Error"),
      },
    );
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <select
          value={filterStatus}
          onChange={(e) => { setFilterStatus(e.target.value as CapturedAnimalStatus | ""); setPage(1); }}
      ...
      {showNewForm && (
        <div className="rounded-2xl border border-trust-200 bg-trust-50 p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            {canMultiCanton && (
              <div>
                ...
                <Input value={form.canton} onChange={(e) => setForm((f) => ({ ...f, canton: e.target.value }))} placeholder={canton} />
              </div>
            )}
            <div>
              <label className="mb-1 block text-xs font-medium text-sand-600">Especie *</label>
              <Input value={form.species} onChange={(e) => setForm((f) => ({ ...f, species: e.target.value }))} placeholder="Perro, Gato…" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-sand-600">Color *</label>
              <Input value={form.color} onChange={(e) => setForm((f) => ({ ...f, color: e.target.value }))} placeholder="Café, negro…" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-sand-600">Raza</label>
              <Input value={form.breed} onChange={(e) => setForm((f) => ({ ...f, breed: e.target.value }))} placeholder="Opcional" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-sand-600">Edad estimada</label>
              <Input value={form.estimatedAge} onChange={(e) => setForm((f) => ({ ...f, estimatedAge: e.target.value }))} placeholder="1-2 años" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-sand-600">N° chip/collar</label>
              <Input value={form.collarChipNumber} onChange={(e) => setForm((f) => ({ ...f, collarChipNumber: e.target.value }))} />
            </div>
          </div>
          <div>
          ...
          <div className="flex gap-2">
            <Button onClick={handleRecord} loading={recordCapture.isPending} className="flex-1">Guardar</Button>
      ...
      {isLoading ? (
        <div className="animate-pulse space-y-2">
          {[1, 2, 3].map((i) => <div key={i} className="h-16 rounded-xl bg-sand-100" />)}
        </div>
            ...
            {data.items.map((a) => (
              <li key={a.id} className="rounded-xl border border-sand-100 bg-surface-warm px-4 py-3">
                <div className="flex items-start gap-3">
                  ...
                  {a.photoUrl && (
                    <img src={a.photoUrl} alt={a.species} className="h-12 w-12 shrink-0 rounded-lg object-cover border border-sand-200" />
                  )}
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="font-semibold text-sand-900">{a.species}</span>
                      {a.breed && <span className="text-xs text-sand-500">{a.breed}</span>}
                      <span className="rounded-full bg-sand-100 px-2 py-0.5 text-xs font-medium text-sand-600">{a.color}</span>
                      <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${
                        a.status === "Received" ? "bg-warn-100 text-warn-700" :
                        a.status === "OwnerFound" ? "bg-rescue-100 text-rescue-700" :
                    ...
                    {a.notes && <p className="mt-1 text-xs text-sand-600 truncate">{a.notes}</p>}
                  </div>
                  <select
                    value={a.status}
                    onChange={(e) => updateStatus.mutate({ id: a.id, status: e.target.value as CapturedAnimalStatus })}
          ...
          <div className="flex items-center justify-between text-xs text-sand-500">
            ...
            <div className="flex gap-2">
              <button type="button" disabled={page === 1} onClick={() => setPage((p) => p - 1)}
                className="rounded px-2 py-1 border border-sand-200 disabled:opacity-40 hover:bg-sand-100">← Anterior</button>
              <button type="button" disabled={page * 20 >= data.total} onClick={() => setPage((p) => p + 1)}
                className="rounded px-2 py-1 border border-sand-200 disabled:opacity-40 hover:bg-sand-100">Siguiente →</button>
            </div>
          </div>
        </>
      ) : (
        <Card padding="sm">
          <p className="text-center text-sm text-sand-400">No hay registros con los filtros actuales.</p>
        </Card>
      )}
    </div>
  );
...
function StatsTab({ tier }: { tier: MunicipalTier }) {
  ...
  if (isLoading)
    return <div className="animate-pulse space-y-2">{[1, 2, 3].map((i) => <div key={i} className="h-16 rounded-xl bg-sand-100" />)}</div>;

  if (!stats) return null;

  const cards = [
    { label: "Total capturado", value: stats.totalCaptured, color: "text-sand-900" },
    { label: "En custodia", value: stats.received, color: "text-warn-700" },
    { label: "Dueño localizado", value: stats.ownerFound, color: "text-rescue-700" },
    { label: "Adoptado", value: stats.adopted, color: "text-trust-700" },
    { label: "Tasa recuperación", value: `${stats.recoveryRate}%`, color: "text-brand-700" },
  ];
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-sand-500">Últimos 30 días</p>
        {stats.last30Days.length > 0 ? (
          ...
          <p className="text-xs text-sand-400">Sin actividad en los últimos 30 días.</p>
        )}
      </div>
    </div>
  );
...
function RegionalTab({ tier }: { tier: MunicipalTier }) {
  ...
  if (isLoading)
    return <div className="animate-pulse space-y-2">{[1, 2, 3].map((i) => <div key={i} className="h-12 rounded-xl bg-sand-100" />)}</div>;
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-sand-500">Por cantón</p>
        <ul className="space-y-2">
...
export default function MunicipalDashboardPage() {
    ...
    return (
      <div className="mx-auto max-w-lg px-4 py-12 text-center space-y-3">
        ...
        <p className="text-sm text-sand-500">
          Tu cuenta tiene rol Municipalidad pero aún no tiene un perfil asignado.
          Contacta al equipo de PawTrack CR para activar tu acceso.
        </p>
      </div>
    );
  }
  ...
  const TABS = [
    ...
    { id: "estadisticas" as const, label: "📊 Estadísticas", disabled: tier === "Basica" },
    { id: "regional" as const, label: "🗺️ Regional", disabled: tier !== "RedRegional" },
  ];
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-sand-500">
            ...
            {profile.expiresAt && (
              <> · vence {new Date(profile.expiresAt).toLocaleDateString("es-CR")}</>
            )}
          </p>
        </div>
        <TierBadge tier={tier} />
