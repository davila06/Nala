...
function AlliesTab() {
  ...
  if (isError)
    return <Error msg="No se pudieron cargar las solicitudes de aliados." />;
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <p className="truncate font-semibold text-sand-900 dark:text-sand-100">
                {ally.organizationName}
              </p>
              <p className="text-xs text-sand-500 dark:text-sand-400">
                {ally.allyType} · {ally.coverageLabel}
              </p>
              <p className="mt-1 text-xs text-sand-400 dark:text-sand-500">
                Aplicación:{" "}
...
function ClinicsTab() {
  ...
  if (isLoading) return <Loading />;
  if (isError)
    return <Error msg="No se pudieron cargar las clínicas pendientes." />;
  if (!data || data.length === 0)
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-xs text-sand-400 dark:text-sand-500">
                {clinic.contactEmail}
              </p>
              <p className="mt-1 text-xs text-sand-400 dark:text-sand-500">
                Registro:{" "}
