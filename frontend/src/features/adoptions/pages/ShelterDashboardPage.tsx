    ...
    <div className="flex items-center gap-3 p-4 rounded-xl border border-sand-100 bg-surface hover:shadow-sm transition-shadow">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className="font-semibold text-sm text-ink-800 truncate">{animal.name}</p>
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 ${STATUS_COLORS[animal.status] ?? ""}`}>
          ...
          <button
            onClick={() =>
              markAdopted.mutate(animal.id, {
                onSuccess: () => toast.success(`${animal.name} marcado como adoptado ✓`),
              })
            }
...
export default function ShelterDashboardPage() {
  ...
  return (
    <>
      <Helmet><title>Panel Shelter · PawTrack CR</title></Helmet>

      <div className="mx-auto max-w-3xl px-4 py-8 space-y-6">
        {/* Header */}
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-ink-900">Panel del Shelter</h1>
        ...
        {isLoading ? (
          ...
          <div className="py-16 text-center text-sand-400">
            ...
            <p className="text-sm font-medium">No has publicado animales todavía</p>
            <Link
              to="/shelter/publicar"
