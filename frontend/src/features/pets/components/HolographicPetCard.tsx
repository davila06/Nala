...
export function HolographicPetCard({
  pet,
  index = 0,
}: HolographicPetCardProps) {
  ...
  useEffect(() => {
    ...
    if (permission) {
      permission
        .then((state) => {
          if (state === "granted") cleanup = attach();
        })
        .catch(() => {});
    } else {
      cleanup = attach();
  ...
  return (
    <Link
      ref={cardRef}
      to={
        isLost && pet.activeLostEventId
          ? `/lost/${pet.activeLostEventId}/case`
          : `/pets/${pet.id}`
      }
      aria-label={
        isLost
          ? `Ver sala de búsqueda de ${pet.name}`
          : `Ver detalles de ${pet.name}`
      }
    ...
    >
          ...
          <>
            ...
            <div className="absolute inset-x-0 top-0 flex items-center justify-center gap-1.5 bg-danger-600/90 py-1.5 text-xs font-bold uppercase tracking-widest text-white">
              <span
                ...
                style={{ animation: "pulse-soft 0.8s ease infinite" }}
              />
      ...
      {isLost && (
        <div
          className="pointer-events-none absolute inset-x-0 bottom-0 h-1 bg-gradient-to-r from-danger-400 via-danger-500 to-danger-400"
          style={{ animation: "pulse-soft 2s ease infinite" }}
        />
      )}
    </Link>
  );
