...
import { useActivateSubscription, useCreateSubscription } from '../hooks/useSubscription'
...
const SINPE_NUMBER = '7000-0000' // replace with real SINPE Móvil number via env var
...
export function SinpePaymentModal({ tier, clinicId, onClose, onSuccess }: SinpePaymentModalProps) {
  const [step, setStep] = useState<'confirm' | 'payment' | 'verifying' | 'done'>('confirm')
  const [reference, setReference] = useState<string | null>(null)
  const [error, setError] = useState<string | null>(null)

  const { mutateAsync: createSub, isPending: isCreating } = useCreateSubscription()
  const { mutateAsync: activateSub, isPending: isActivating } = useActivateSubscription()
  ...
  async function handleConfirmPayment() {
    ...
    try {
      ...
      setError('No se pudo verificar el pago. Si ya enviaste el SINPE, espera unos minutos y vuelve a intentarlo.')
      setStep('payment')
    }
  }
      ...
      >
        ...
        >
          ...
          <div className="mb-5 flex items-start justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-brand-500">SINPE Móvil</p>
          ...
          {step === 'payment' && reference && (
            <div className="space-y-4">
              <div className="rounded-2xl bg-surface-warm p-4 text-center">
                <p className="text-xs text-sand-500 mb-1">Código de referencia</p>
                <p className="font-mono text-3xl font-black tracking-[0.2em] text-sand-900">{reference}</p>
              </div>
              <ol className="space-y-3 text-sm text-sand-700">
                <li className="flex gap-3">
                  <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-brand-100 text-[10px] font-bold text-brand-700">1</span>
                  Abre tu app de banco y selecciona <strong>SINPE Móvil</strong>.
                </li>
                <li className="flex gap-3">
                  <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-brand-100 text-[10px] font-bold text-brand-700">2</span>
                  Envía <strong>₡{price.toLocaleString('es-CR')}</strong> al número <strong>{SINPE_NUMBER}</strong>.
                </li>
                <li className="flex gap-3">
                  <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-brand-100 text-[10px] font-bold text-brand-700">3</span>
                  Escribe el código <strong>{reference}</strong> en el campo de descripción/mensaje.
                </li>
                <li className="flex gap-3">
                  <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-brand-100 text-[10px] font-bold text-brand-700">4</span>
          ...
          {step === 'verifying' && (
            <div className="flex flex-col items-center gap-4 py-6">
              ...
              <p className="text-sm font-semibold text-sand-700">Verificando pago…</p>
              <p className="text-xs text-sand-400 text-center">No cierres esta ventana.</p>
            </div>
          )}
          ...
          {step === 'done' && (
            <div className="flex flex-col items-center gap-4 py-4 text-center">
              ...
              <h3 className="text-lg font-black text-sand-900">¡Plan {label} activado!</h3>
              <p className="text-sm text-sand-500">Tu suscripción está activa por 30 días.</p>
              <button
                type="button"
                onClick={onClose}
