      ...
      <div className="rounded-2xl border border-dashed border-sand-200 bg-surface-warm p-6 text-center">
        ...
        {showSetup && (
          <div className="mt-4 rounded-2xl border border-brand-200 bg-surface p-4 text-left space-y-3">
            <p className="text-sm font-semibold text-sand-800">Configurar collar GPS</p>
            <p className="text-xs text-sand-500">
              Compatible con <strong>Tractive</strong>, <strong>Kippy</strong> o cualquier GPS genérico.
              Ingresa el ID del dispositivo impreso en el collar.
            </p>
            <label className="block text-xs font-semibold text-sand-700">
              ID del dispositivo (Tractive, Kippy, etc.)
              <input
                type="text"
                value={deviceId}
                onChange={(e) => setDeviceId(e.target.value)}
                placeholder="Ej: TRACTIVE-ABC123"
            ...
            <div className="flex gap-2">
              <button
                ...
                onClick={async () => {
                  await register({
                    petId,
                    provider: deviceId.toLowerCase().startsWith('tractive') ? 'Tractive' : 'Generic',
              ...
              <button type="button" onClick={() => setShowSetup(false)} className="text-xs text-sand-400 hover:text-sand-600">
                Cancelar
              </button>
            </div>
          </div>
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        {collar.batteryPercent !== null && (
          <div className="flex flex-col items-center">
            <span className={`text-sm font-bold tabular-nums ${collar.batteryPercent < 20 ? 'text-danger-600' : 'text-rescue-600'}`}>
      ...
      {collar.lastLat !== null && collar.lastLng !== null ? (
        <div className="overflow-hidden rounded-2xl border border-sand-200" style={{ height: 260 }}>
          <MapContainer
          ...
          >
            ...
            <Marker position={[collar.lastLat, collar.lastLng]}>
              <Popup>
                Última posición<br />
                {collar.lastSeenAt && new Date(collar.lastSeenAt).toLocaleString('es-CR')}
              </Popup>
            </Marker>
          </MapContainer>
        </div>
      ...
      <p className="text-center text-[10px] text-sand-400">
        La posición se actualiza automáticamente cada 30 segundos.
        La integración completa con Tractive OAuth requiere configuración adicional.
      </p>
    </div>
  )
}
