...
export function usePublicMapEvents(bbox: MapBBox | null) {
  ...
  return useQuery({
    ...
    staleTime: 30_000,
    refetchInterval: 60_000, // keep the "live map" fresh every 60s
    refetchOnWindowFocus: false,
  });
}

/**
 * Returns a debounced bbox setter (150 ms) so the map query only fires
 * after the user stops panning/zooming. moveend already fires once the
 * drag finishes, so 150 ms is enough to batch rapid programmatic moves
 * without adding perceptible latency.
 */
export function useDebouncedBBox(delay = 150) {
