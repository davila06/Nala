...
import { useActivityLogs, useLogActivity, useDeleteActivity } from "../hooks/useActivity";
...
const TYPE_CONFIG: Record<ActivityType, { label: string; emoji: string; color: string }> = {
  ...
  Training: { label: "Entrenamiento", emoji: "🏋️", color: "var(--color-warn-600)" },
...
function QuickLogForm({ petId, onClose }: { petId: string; onClose: () => void }) {
    ...
    logActivity.mutate(
      {
        ...
        distanceMeters: distance ? Math.round(parseFloat(distance) * 1000) : undefined,
        notes: notes.trim() || undefined,
      },
      {
        onSuccess: () => { toast.success("Actividad registrada"); onClose(); },
  ...
  return (
    <div className="rounded-2xl border border-brand-200 bg-brand-50 p-4 space-y-4">
      <h3 className="text-sm font-semibold text-brand-800">Registrar actividad</h3>
      ...
      <div>
        <label className="mb-1 block text-xs font-medium text-sand-600">
          Duración: <span className="font-bold text-sand-900">{duration} min</span>
        </label>
        <input
          type="range"
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label htmlFor="act-distance" className="mb-1 block text-xs font-medium text-sand-600">
        ...
        <div>
          <label htmlFor="act-date" className="mb-1 block text-xs font-medium text-sand-600">
            Fecha
          </label>
          <input
            id="act-date"
...
function WeeklyChart({ data }: { data: { day: string; minutes: number; type: ActivityType }[] }) {
  return (
    <ResponsiveContainer width="100%" height={140}>
      <BarChart data={data} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
        <CartesianGrid strokeDasharray="3 3" stroke="var(--color-sand-200)" vertical={false} />
        <XAxis dataKey="day" tick={{ fontSize: 10, fill: "var(--color-sand-500)" }} axisLine={false} tickLine={false} />
        <YAxis tick={{ fontSize: 10, fill: "var(--color-sand-500)" }} axisLine={false} tickLine={false} unit=" m" />
        <Tooltip
          formatter={(v: number) => [`${v} min`, "Duración"]}
          contentStyle={{ borderRadius: 12, border: "1px solid var(--color-sand-200)", fontSize: 11 }}
        />
        <Bar dataKey="minutes" radius={[4, 4, 0, 0]}>
          {data.map((entry, i) => (
            <Cell key={i} fill={entry.minutes > 0 ? "var(--color-brand-500)" : "var(--color-sand-200)"} />
          ))}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
...
export function ActivityTab({ petId, petName }: ActivityTabProps) {
  ...
  if (tierLoading || isLoading) return <Skeleton className="h-48 w-full rounded-2xl" />;
    ...
    return (
      <div className="rounded-2xl border border-sand-200 bg-surface p-6 text-center space-y-3">
        ...
        <p className="text-sm text-sand-500">
          Disponible en el plan Plus. Lleva el historial de ejercicio de {petName} y conecta con tu collar Tractive.
        </p>
      </div>
    );
  }
  ...
  const thisWeekMin = data?.weeklyTotals[data.weeklyTotals.length - 1]?.totalMinutes ?? 0;
  const benchmark = data?.benchmark;
  const streakPct = benchmark ? Math.min(100, Math.round(thisWeekMin / 7 / benchmark.dailyMinutesMax * 100)) : 0;
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-bold text-sand-900">
              {data?.streakDays ?? 0} día{data?.streakDays !== 1 ? "s" : ""} consecutivos
            </p>
            <p className="text-xs text-sand-500">Racha actual de {petName}</p>
          </div>
        </div>
        <Button size="sm" onClick={() => setShowForm(true)} aria-label="Registrar nueva actividad">
          + Registrar
        </Button>
      </div>

      {showForm && <QuickLogForm petId={petId} onClose={() => setShowForm(false)} />}
        ...
        <div className="rounded-xl border border-sand-100 bg-sand-50 p-4 space-y-2">
          <div className="flex items-center justify-between text-xs">
            <span className="font-semibold text-sand-700">
              Objetivo semanal: {benchmark.dailyMinutesMin * 7}–{benchmark.dailyMinutesMax * 7} min
            </span>
            <span className={`font-bold ${
              streakPct >= 80 ? "text-rescue-600" : streakPct >= 50 ? "text-warn-600" : "text-danger-600"
            }`}>
              {thisWeekMin} min esta semana
            </span>
          </div>
          <div className="h-2 rounded-full bg-sand-200 overflow-hidden" role="progressbar" aria-valuenow={streakPct} aria-valuemin={0} aria-valuemax={100}>
            <div
              className={`h-full rounded-full transition-all ${
                streakPct >= 80 ? "bg-rescue-500" : streakPct >= 50 ? "bg-warn-500" : "bg-danger-500"
              }`}
      ...
      {/* 7-day chart */}
      <div className="rounded-xl border border-sand-100 bg-surface p-4" role="img" aria-label={`Actividad de ${petName} en los últimos 7 días`}>
        <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-sand-500">Últimos 7 días (min)</p>
      ...
      {(data?.logs.length ?? 0) === 0 ? (
        ...
        <ul className="space-y-2" aria-label={`Historial de actividad de ${petName}`}>
          {data!.logs.map((log) => (
            <li
              key={log.id}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-sand-500">
                  ...
                  {log.distanceMeters != null ? ` · ${(log.distanceMeters / 1000).toFixed(1)} km` : ""}
                </p>
              </div>
              {log.source !== "Tractive" && (
                <button
