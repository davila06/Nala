  ...
  return (
    ...
    >
      {reminder.petPhotoUrl ? (
        <img
          src={reminder.petPhotoUrl}
          alt={reminder.petName}
          className="h-9 w-9 rounded-full object-cover shrink-0"
        />
      ) : (
        <div className="h-9 w-9 rounded-full bg-sand-100 flex items-center justify-center text-lg shrink-0">
          🐾
        </div>
      )}
      ...
      {overdueCount > 0 && (
        <div className="flex items-center gap-2 rounded-xl bg-danger-50 border border-danger-200 px-3 py-2">
          <span className="text-sm font-semibold text-danger-700">
            ⚠️ {overdueCount} recordatorio{overdueCount !== 1 ? "s" : ""}{" "}
            vencido{overdueCount !== 1 ? "s" : ""}
          </span>
        </div>
      )}
        ...
        <div key={bucket}>
          ...
          <div className="space-y-2">
            {grouped.get(bucket)!.map((r) => (
              <ReminderRow key={r.reminderId} reminder={r} />
            ))}
          </div>
        </div>
      ))}
