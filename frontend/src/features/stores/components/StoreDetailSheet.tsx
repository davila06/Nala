...
export function StoreDetailSheet({ storeId, isOpen, onClose, onCheckout }: StoreDetailSheetProps) {
  ...
  const handleAdd = (product: NonNullable<typeof data>["products"][number]) => {
    if (cartStoreId && cartStoreId !== storeId) {
      if (!window.confirm("Tu carrito tiene productos de otra tienda. ¿Deseas vaciarlo y agregar este?")) return;
    }
  ...
  return (
    <Drawer isOpen={isOpen} onClose={onClose} title={data?.store.name ?? "Tienda"} side="bottom" maxWidth={540}>
      <div className="space-y-4 pb-safe">
        {isLoading && (
          <div className="space-y-3 animate-pulse">
            {[...Array(3)].map((_, i) => <div key={i} className="h-16 rounded-xl bg-sand-100" />)}
          </div>
        )}
        ...
        {data && (
          <>
            ...
            {data.products.length === 0 ? (
              <p className="py-8 text-center text-sm text-sand-400">Esta tienda no tiene productos disponibles aún.</p>
            ) : (
              (() => {
                const grouped = data.products.reduce<Record<string, typeof data.products>>((acc, p) => {
                ...
                return Object.entries(grouped).map(([cat, prods]) => (
                  <div key={cat}>
                    ...
                    <ul className="space-y-2">
                      {prods.map((product) => (
                        <li key={product.id} className="flex items-center gap-3 rounded-xl border border-sand-100 bg-surface p-3">
                          {product.imageUrl && (
                            <img src={product.imageUrl} alt={product.name} className="h-12 w-12 rounded-lg object-cover border border-sand-100 shrink-0" />
                          )}
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-semibold text-sand-900">{product.name}</p>
                            {product.description && <p className="text-xs text-sand-500 truncate">{product.description}</p>}
            ...
            {cartCount > 0 && (
              <div className="sticky bottom-0 bg-surface pt-3 pb-1">
                <Button fullWidth onClick={() => { onClose(); onCheckout(); }}>
