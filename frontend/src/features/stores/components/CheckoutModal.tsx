...
export function CheckoutModal({ isOpen, onClose }: CheckoutModalProps) {
  ...
  const handlePlaceOrder = () => {
    ...
    placeOrder.mutate(
      ...
      {
        onSuccess: (data) => { setOrder(data); setStep("payment"); clear(); },
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-sand-100">
            <h2 className="font-display text-base font-semibold text-sand-900">
              {step === "form" ? "Confirmar pedido" : step === "payment" ? "Realizar pago SINPE" : "¡Pedido enviado!"}
            </h2>
          ...
          <div className="px-5 py-5 space-y-4">
            ...
            {step === "form" && (
              <>
                ...
                <div className="rounded-xl border border-sand-100 bg-sand-50 p-4 space-y-2">
                  {items.map((i) => (
                    <div key={i.product.id} className="flex items-center justify-between text-sm">
                      <span className="text-sand-700">{i.product.name} × {i.quantity}</span>
                      <span className="font-semibold text-sand-900">₡{(i.product.priceCrc * i.quantity).toLocaleString("es-CR")}</span>
                    </div>
                  ))}
                ...
                {/* Fulfillment type */}
                <div>
                  <p className="mb-2 text-xs font-medium text-sand-600">Tipo de entrega</p>
                ...
                {fulfillment === "Delivery" && (
                  <div>
                    <label className="mb-1 block text-xs font-medium text-sand-600">Dirección de entrega *</label>
                    <Input value={deliveryAddress} onChange={(e) => setDeliveryAddress(e.target.value)} placeholder="200m norte del parque..." />
                  </div>
                )}

                <div>
                  <label className="mb-1 block text-xs font-medium text-sand-600">Nota para la tienda (opcional)</label>
                  <Input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Sin gluten, alérgico a..." />
                </div>
            ...
            {step === "payment" && order && (
              <div className="space-y-4">
                <div className="rounded-2xl border border-warn-200 bg-warn-50 p-5 space-y-3 text-center">
                  <p className="text-xs font-semibold uppercase tracking-wider text-warn-600">Referencia SINPE Móvil</p>
                  <p className="font-mono text-3xl font-black tracking-widest text-sand-900">{order.paymentReference}</p>
                  <button type="button" onClick={() => { navigator.clipboard.writeText(order.paymentReference); toast.success("Referencia copiada"); }}
                ...
                <p className="text-center text-sm text-sand-500">
                  Realiza el SINPE Móvil y toca <strong>"Ya pagué"</strong> cuando termines.
                  La tienda confirmará tu pedido en breve.
                </p>
                <p className="text-center font-bold text-sand-700">
                  Monto: ₡{order.totalCrc.toLocaleString("es-CR")}
                </p>
                <Button fullWidth onClick={handleReportPayment} loading={reportPayment.isPending}>
                  ✓ Ya realicé el pago
                </Button>
              </div>
            )}
            ...
            {step === "done" && (
              <div className="space-y-4 text-center py-4">
                ...
                <h3 className="font-display text-xl font-bold text-sand-900">¡Pago reportado!</h3>
                <p className="text-sm text-sand-500">
                  La tienda verificará tu pago y confirmará el pedido. Recibirás una notificación.
                </p>
                <Button fullWidth variant="secondary" onClick={handleClose}>Cerrar</Button>
              </div>
            )}
          </div>
        </motion.div>
