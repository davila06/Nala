...
export type QueuedReportStatus = "pending" | "retrying" | "conflict" | "failed" | "done";
export type OfflineFailureDisposition = "retry" | "conflict" | "failed";
export const MAX_OFFLINE_RETRIES = 7;
...
export interface QueuedReport {
  ...
  /** Safe user-facing reason when automatic delivery stops. */
...
export async function markQueuedReportFailed(
  id: string,
...
export async function resetQueuedReportForRetry(id: string): Promise<void> {
  const db = await getDb();
  const record = await db.get("report-queue", id);
  if (!record) return;
  await db.put("report-queue", {
...
export function classifyOfflineFailure(
  ...
  if (nextRetryCount >= MAX_OFFLINE_RETRIES) return "failed";
  if (httpStatus === undefined || httpStatus === 408 || httpStatus === 425 || httpStatus === 429 || httpStatus >= 500)
    return "retry";
  return "failed";
}
